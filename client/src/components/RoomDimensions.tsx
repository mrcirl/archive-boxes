...
import { getRoomPolygonPoints } from '../geometry/roomPolygon';
...
function getWallSegments(scan: Scan) {
  const points = getRoomPolygonPoints(scan);
...
function WallLengthLabels({
  ...
  return (
    <>
      {segments.map((seg, i) => {
        // Nudged just off the wall face (not floating away from it) so the
        // label reads as attached to that specific wall.
        ...
        return (
          <Html key={i} position={[labelX, y, labelZ]} center zIndexRange={[0, 0]}>
            <div className="dimension-label wall-label">
              {seg.length.toFixed(2)} × {wallHeightM.toFixed(2)} m
            </div>
          </Html>
        );
      })}
 ...
 * per-wall length×height labels are shown directly on each wall
 * (WallLengthLabels) instead of the generic axis-aligned bounding-box
 * measurement — for a rotated/irregular room those two would disagree and
 * drawing both would just duplicate (and for a plain rectangle, exactly
 * overlap) the same information. Scans without known wall geometry
 * (uploads, USDZ conversions) fall back to the bounding-box-only display,
 * clearly labeled as such since it isn't the real footprint or individual
 * wall lengths. */
